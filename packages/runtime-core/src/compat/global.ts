...
export type CompatVue = Pick<App, 'version' | 'component' | 'directive'> & {
  ...
  mixin(mixin: any): CompatVue

  component(name: string): any | undefined
  component(name: string, component: any): CompatVue
  directive(name: string): any | undefined
  directive(name: string, directive: any): CompatVue
...
export function createCompatVue(
  ...
  function createCompatApp(options: any = {}, Ctor: any) {
  ...
  Vue.directive = ((name: string, dir: any | LegacyDirective) => {
    if (dir) {
      singletonApp.directive(name, dir as any)
      return Vue
    } else {
  ...
  function extendCtor(this: any, extendOptions: any = {}) {
    ...
    const Super = this
    function SubVue(inlineOptions?: any) {
      if (!inlineOptions) {
  ...
  const util = {
    warn: NOOP,
    extend,
    mergeOptions: (parent: any, child: any, vm?: any) =>
      mergeOptions(
        parent,
        child,
...
function installFilterMethod(app: App, context: AppContext) {
  ...
  app.filter = (name: string, filter?: Function): any => {
    ...
    if (__DEV__ && context.filters![name]) {
      console.warn(`Filter "${name}" has already been registered.`)
    }
...
function installCompatMount(
  ...
  app._createRoot = options => {
    ...
    const vnode:any = createVNode(component, options.propsData || null)
    ...
    instance.ctx._compat_mount = (selectorOrEl?: string | Element) => {
      if (isMounted) {
        __DEV__ && console.warn(`Root instance is already mounted.`)
        return
      }
      ...
      if (hasNoRender && instance.render === emptyRender) {
        ...
        instance.render = null
          ; (component as any).template = container.innerHTML
