...
import { formatComponentName, getComponentName, getCurrentInstance } from '../component'
  ...
  for (const key of Object.keys(config)) {
    if (
      ...
      if (key.startsWith('COMPILER_')) {
        ...
        console.warn(`Invalid deprecation config "${key}".`)
      }
...
export function getCompatConfigForKey(
  ...
  const instanceConfig =
    instance && (instance.type as any).compatConfig
...
export function warnDeprecation(
  ...
  if (!__TEST__ && dupKey in warnCount) {
    console.warn(`(deprecation ${key}) (${++warnCount[dupKey] + 1})`)
    return
  }
